 ...
 \foreach \x [count=\xcount] in {1,3,5}
  {
	...
	\pgfmathsetmacro{\adjustedindex}{\intcalcMod{\currentindex + 6}{9}}
      ...
      \node (\x \y) at (\x,\y) {$X_\adjustedindex$};
      \ifnum\y>1
   ...
   \foreach \x [count=\xcount] in {1,3,5}
  {
    ...
    \pgfmathsetmacro{\currentindex}{\pgfmathresult}
      ...
      \node (\x \y) at (\x,\y) {$X_{\currentindex}$};
      \ifnum\y>1
        \pgfmathparse{int(\y-2)}                             
        \draw  (\x \y) -- (\x \pgfmathresult);
      \fi 
...
If we parametrize the joint $p(\vek{x}) = \frac{1}{Z} \prod_C \psi_{c}\vek(x_C)$ the same as before with one parameter per clique the 8-neighbourhood model will be heavily biased due to an overall decrease in the number of parameters with respect to the model complexity. Therefore we need to introduce parameters for the different connections within cliques.
